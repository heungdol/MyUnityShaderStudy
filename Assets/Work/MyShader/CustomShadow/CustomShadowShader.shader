    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (vertOutput output) : SV_TARGET
            {
                ...
                //float3 world2Obj1 = mul (float4 (output.worldPos, 1), unity_ObjectToWorld);

                float3 world2Obj0 = mul (unity_WorldToObject, output.worldPos);
                
                //float scaleZ = world2Obj.z - output.worldPos.z;
                float patternCol = tex2D (_PatternTex, world2Obj0 * _PatternScale).r;
                
                if (patternCol < (1 - s))
                    discard;

                return float4 (_PatternCol.rgb, 1);
            }
            ENDCG
        }
    }
